...
export async function GET() {
  ...
  try {
    ...
    const presets: GuidelinesPreset[] = [];
    for (const file of files.filter((f) => f.endsWith(".json"))) {
      try {
        const raw = await readFile(path.join(DIR, file), "utf-8");
        presets.push(JSON.parse(raw) as GuidelinesPreset);
      } catch { /* skip malformed */ }
    }
    return NextResponse.json({ guidelines: presets });
  } catch {
...
export async function POST(req: NextRequest) {
  await ensureDir();
  const preset = (await req.json()) as GuidelinesPreset;
  const safeId = preset.name
